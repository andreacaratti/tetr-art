...
class Lvl2TheStarryNight(level):
    def __init__(self):
        super().__init__()
        self.size = Vector2(16,16)
        self.SIZE_FACTOR = 0.3 # 0.4
        self.PIXELS_PER_TILE = 3
        self.duration = 480
    
    def load_image(self):
        level_image_path = path.join(base_path,"data","img","levels","lvl2_TheStarryNight_Pixel.png")
        image = pygame.image.load(level_image_path).convert()
        self.split_image(image=image)

        level_preview_path = path.join(base_path,"data","img","levels","lvl2_TheStarryNight_Pixel.png")
